...
public class BNode implements BNodeInterface {
	...
	{
		int i = 1;
		while (i<getNumOfBlocks() & key > getBlocksList().get(i).getKey())
			i++;
		if(i<= getNumOfBlocks() & key == getBlocksList().get(i).getKey())
			return getBlocksList().get(i);
		else if(isLeaf())
			return null;
	...
	{
		int i = getBlocksList().size();
		if(isLeaf)
		{
			while(i>=1 & d.getKey() < getBlockKeyAt(i))
			{
				blocksList.set(i+1, getBlockAt(i));
				i--;
			}
